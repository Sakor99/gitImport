...
write_config () {
  ...
  ckan config-tool -e $CONFIG \
    ...
    "ckan.plugins = gitimport stats text_view image_view recline_view resource_proxy officedocs_view webpage_view videoviewer TIBtheme dcat dcat_json_interface pdf_view scheming_datasets tibimport jupyternotebook tibvocparser scheming_tibupdateresources tib_cadviewer" \
  ...
  ckan config-tool -s app:main $CONFIG "scheming.dataset_schemas = ckanext.scheming:ckan_dataset.yaml ckanext.scheming:ckan_vdataset.yaml ckanext.scheming:service.yaml ckanext.scheming:ckan_github.yaml"
